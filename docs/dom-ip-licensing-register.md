...
not approving it** - and the project reviewer approved these rows on 2026-09-03,
which is a separate act by a separate person.

`Approved` below means this project judged the recorded evidence sufficient to
proceed and nothing more. It is not a clearance, not an opinion, and not a
warranty. The wording rules DOM-IP-007 sets are unchanged by any of it: this
component is still not to be described as HTML5-conformant, spec-compliant, or as
a browser engine.
...
| Blocker | Kind | State |
|---|---|---|
| DOM-IP-001 | Two royalty-free patent policies | **Approved 2026-09-03.** The exclusion mechanism is recorded as what stops it being a patent-freedom claim |
| DOM-IP-002 to DOM-IP-004, DOM-IP-006, DOM-SRC-001 | What this repository contains and consulted | **Approved 2026-09-03.** Three guarded mechanically, DOM-IP-003 among them |
| DOM-IP-005 | Third-party conformance suites | **Rejected by default.** Guarded by extension |
| DOM-IP-007 wording | How the component is described | **Approved 2026-09-03.** The negative half was enforced before the decision and is unchanged by it |

## Review record

| Review | Reviewer | Date | Scope | Result |
|---|---|---|---|---|
| Patent, provenance and wording review (all rows) | Project reviewer (Maik Ratzmer), on the evidence record assembled 2026-09-03 | 2026-09-03 | The WHATWG IPR Policy and W3C Patent Policy as published, including the copyright terms of Living Standards; this repository's two libraries, their tests and their project references | **Approved.** The row that carried this register is DOM-IP-003, and it is the one an approval is worth least on and most easily kept honest: the named character reference table is not here, two call sites hand the work to the platform, and a guard fails the build if a transcribed table appears - so what was approved is a finding that cannot quietly stop being true. DOM-IP-002 is accepted for what it is rather than what it permits: WHATWG licenses its standards under CC BY 4.0 and BSD-3 where text is incorporated into source, which answers the transcription question that gates the PDF fax decoder under SRC-017 elsewhere in the platform, and this component does not need the answer because it reproduces nothing. **Not decided by any of it:** patent-freedom, which is claimed nowhere, and freedom to operate, for which nobody searched. DOM-IP-005 is untouched and is not an approval - third-party conformance suites, html5lib-tests among them, stay rejected by default per artifact. |
